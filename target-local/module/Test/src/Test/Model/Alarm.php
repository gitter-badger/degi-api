    ...
    public function selectAll($query){ 
   	 	try {
   	 		if( $query['put'] == 'true'){
   	 			...
   	 			$data['al_button'] = 1;
   	 			$data['al_created'] = date('Y-m-d H:i:s',(time()+42237));
   	 			$this->db->insert($data);
				...
				// 瓦斯濃度過高 
	   	 		...
	   	 		if( $im_array['now']['location'] == 'Bathroom' ){
	   	 			...
	   	 			if( $im_array['now']['timeDiff'] > (float)$shower_avg * (1.4) ){
	   	 				//警告! 用戶 Mary 已待在浴室 20 分鐘 ( 現在時間: 08:44:00 ) , 超過平常洗澡時間平均值四成!
	   	 				$message .= '警告! 用戶 Mary 已待在浴室 '.$im_array['now']['timeDiff'].'分鐘 ( 現在時間：'.date('H:i:s',(time()+42237)).') , 超過平常洗澡時間平均值四成!';
		   	 			$result['alarm'] = true ;
		   	 			$result['msg'] = $message;
		   	 			$data = array();
		   	 			$data['a_id'] = $query['a_id'];
	   	 		...
	   	 		$select->where('`al_button`=1 && `al_created` > DATE_SUB(now()+INTERVAL 42237 SECOND, INTERVAL 10 MINUTE)');
	   	 		$select->where('a_id='.$query['a_id']);
	   	 		$select->limit(1);
	   	 		if( count($this->db->selectWith($select)->toArray()) != 0 ){
		   	 		$start = strtotime($this->db->selectWith($select)->toArray()[0]['al_created']);
		   	 		$end = strtotime(date('Y-m-d H:i:s',(time()+42237)));
	   		 		$timeDiff = $end - $start;
	   		 		//警告! 用戶 Mary 在 08:44:00 在 Bathroom 按下緊急按鈕!
	   		 		$message .= '警告! 用戶 Mary 在 '.date("h:i:s",$this->db->selectWith($select)->toArray()[0]['al_created']).' 在 '.$im_array['now']['location'].' 按下緊急按鈕!';
	   	 			$data = array();
	   	 			$data['al_button'] = 0;
	   	 			$data['al_msg'] = $message;
